<?php
/**
 * MediHub Installation Script - Fixed Version
...
?>";
        ...
        case 3:
            ...
            try {
                ...
                // Check if migrate.sql exists
                if (!file_exists('scripts/migrate.sql')) {
                    throw new Exception('Migration file scripts/migrate.sql not found');
                }
                ...
                if (count($errors) > 0) {
                    $error = 'Some statements failed: ' . implode('; ', $errors);
                } else {
                    $success = "Database schema created successfully! ($executed statements executed)";
                    $step = 4;
                }
            } catch (Exception $e) {
        ...
        case 4:
            ...
            try {
                ...
                if (!file_exists('scripts/seed.sql')) {
                    throw new Exception('Seed file scripts/seed.sql not found');
                }
                ...
                if (count($errors) > 0) {
                    $error = 'Some seed statements failed: ' . implode('; ', $errors);
                } else {
                    // Create installed flag
                    file_put_contents('config/installed.flag', date('Y-m-d H:i:s'));
                    
                    $success = "MediHub installed successfully! ($executed statements executed)";
                    $step = 5;
                }
            } catch (Exception $e) {
